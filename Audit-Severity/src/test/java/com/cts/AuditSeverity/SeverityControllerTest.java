...
public class SeverityControllerTest {
	...
	public void testAuditSeverity() {
		...
		assertNotNull(severityController.auditSeverity( new AuditRequest("P","Q","R",new AuditDetails("Internal",null))).getBody());
	}
	
	@Test
	public void testAuditSeverityTokenFails() {
		when(tokenService.checkTokenValidity("token")).thenReturn(false);
		assertEquals(HttpStatus.FORBIDDEN,severityController.auditSeverity(new AuditRequest("P","Q","R",new AuditDetails("Internal",null))).getStatusCode());
